...
fn moves_down_proportionally_to_elapsed_time() {
    ...
    let s = m.tick(3_500, TravelProfile::linear(UP_MS, DOWN_MS)); // 2.5s of a 10s run = 25%
...
fn moves_up_proportionally_from_partial_position() {
    ...
    let s = m.tick(2_000, TravelProfile::linear(UP_MS, DOWN_MS)); // 2s of 10s = 20% traveled up
...
fn snaps_to_target_and_reports_arrival_once() {
    ...
    let s = m.tick(5_000, TravelProfile::linear(UP_MS, DOWN_MS));
    assert_eq!(s.pos, Pos::from_percent(50));
    assert!(s.arrived);
    let s2 = m.tick(6_000, TravelProfile::linear(UP_MS, DOWN_MS)); // stays put after arrival
...
fn overshoot_clamps_to_target_not_past() {
    let mut m = Motion::new(Pos::ZERO);
    m.set_target(Pos::from_percent(50), 0);
    let s = m.tick(60_000, TravelProfile::linear(UP_MS, DOWN_MS)); // way past arrival time
    assert_eq!(s.pos, Pos::from_percent(50));
}
...
fn reversal_mid_travel_uses_live_position_as_new_start() {
    ...
    m.tick(2_000, TravelProfile::linear(UP_MS, DOWN_MS)); // at 20%
    m.set_target(Pos::ZERO, 2_000); // reverse toward open
    let s = m.tick(3_000, TravelProfile::linear(UP_MS, DOWN_MS)); // 1s up from 20% = 10%
...
fn halt_freezes_position() {
    let mut m = Motion::new(Pos::ZERO);
    m.set_target(Pos::FULL, 0);
    m.tick(4_000, TravelProfile::linear(UP_MS, DOWN_MS));
    m.halt(4_000, TravelProfile::linear(UP_MS, DOWN_MS));
    let s = m.tick(9_000, TravelProfile::linear(UP_MS, DOWN_MS));
...
fn zero_travel_time_jumps_instantly() {
    ...
    let mut m = Motion::new(Pos::ZERO);
    m.set_target(Pos::FULL, 0);
    let s = m.tick(1, TravelProfile::linear(UP_MS, 0));
...
fn asymmetric_travel_times_use_direction_specific_time() {
    ...
    let s = m.tick(2_500, TravelProfile::linear(5_000, 20_000)); // 2.5s of 5s up = 50% traveled
    assert_eq!(s.pos, Pos::from_percent(50));
}
