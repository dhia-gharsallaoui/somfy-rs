...
| [`somfy-mqtt`](crates/somfy-mqtt) | yes | MQTT topic construction, configuration validation and Home Assistant discovery payloads. The discovery prefix and the state root are separate types that hold their text privately, so concatenating them — the fault that made discovery unusable on the C++ build — does not compile. Bad configuration is refused with a typed error naming the field, never repaired. Payload and publisher are derived from one topic table so they cannot drift apart. Network-free. |
