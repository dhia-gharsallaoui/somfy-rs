...
| WiFi (Plan 5) | [`esp-radio`](https://crates.io/crates/esp-radio) 0.18.0, [`esp-wifi`](https://crates.io/crates/esp-wifi) 0.15.1 | **Adopt `esp-radio` 0.18.0.** Verified against the real graph: it resolves with our `esp-hal 1.1.2` (`cargo tree -i esp-hal` stays at 1.1.2) and comes from the same repo, so it tracks the HAL. Note it requires a heap — `build-std` must gain `alloc`. `cargo add` skips `1.0.0-beta.0` as a pre-release. |
...
| MQTT client (Plan 5) | [`rust-mqtt`](https://crates.io/crates/rust-mqtt) 0.5.1, [`minimq`](https://crates.io/crates/minimq) 0.13.0 | **Adopt one — decision deferred to Plan 5 Task 3, with the comparison recorded here.** Both `no_std`, maintained, and MIT/Apache-2.0 (compatible with our GPL-3.0-only). Do **not** hand-roll a client: the hard parts of the spec are topic construction, retention and lifecycle, not packet framing. |
